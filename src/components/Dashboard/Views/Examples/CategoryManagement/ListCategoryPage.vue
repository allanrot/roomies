    ...
    <div class="col-md-12 card mt-3">
      <div class="card-header">
        ...
        <div class="text-right mb-3">
          <p-button @click="goToAdd()" class="mt-3" type="primary">Add Category</p-button>
        </div>
      </div>
...
<script>
  ...
  methods: {
    ...
    async getList() {
      ...
      await this.$store.dispatch("categories/list", params).then(() => {
        this.categories = this.$store.getters["categories/list"];
    ...
    async deleteCategory(id) {
      ...
      try {
        ...
        if (error.response.data.errors[0]) {
          this.$notify({
            type: "danger",
            message: "This Category still has associated Items.",
            icon: 'nc-icon nc-bell-55',
          });
        } else {
          this.$notify({
